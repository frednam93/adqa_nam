...
OUT_BASE=${ROOT}/outputs/grpo_minimal_hard_base
JUDGE_LINK_DIR=${ROOT}/outputs/grpo_minimal_hard_base_judge_inputs
JUDGE_OUT=${ROOT}/outputs/analysis/rescore_choice_matching/grpo_minimal_hard_base_qwen3_judge
...
run_grpo() {
  ...
  cd "${ROOT}"
  echo "==== $(date '+%m%d %H:%M:%S') base-${run} train start ===="
...
run_eval() {
  ...
  local output=${ROOT}/outputs/qwen3_audio_dep_base_hard_grpo_${run}_dev_ckpt100.jsonl
  cd "${ROOT}"
  echo "==== $(date '+%m%d %H:%M:%S') base-${run} eval start ===="
...
echo "==== $(date '+%m%d %H:%M:%S') base GRPO smoke start ===="
python3 -m dcase_adqa.train_qwen3_omni_grpo \
  --manifest "${GRPO_MANIFEST}" \
  --model "${MODEL}" \
  ...
  --save-steps 3 \
  --log-steps 1
notify "DCASE base-GRPO smoke done" "starting base grpo100 and dapo-lite100"

run_grpo grpo100
run_eval grpo100
notify "DCASE base GRPO100 done" "$(summarize_eval_file "${ROOT}/outputs/qwen3_audio_dep_base_hard_grpo_grpo100_dev_ckpt100.jsonl")"
sleep 3

run_grpo dapo_lite100 --dapo-lite
run_eval dapo_lite100
notify "DCASE base DAPO-lite100 done" "$(summarize_eval_file "${ROOT}/outputs/qwen3_audio_dep_base_hard_grpo_dapo_lite100_dev_ckpt100.jsonl")"
sleep 3

rm -rf "${JUDGE_LINK_DIR}"
mkdir -p "${JUDGE_LINK_DIR}" "${JUDGE_OUT}"
ln -sf "${ROOT}/outputs/qwen3_audio_dep_base_hard_grpo_grpo100_dev_ckpt100.jsonl" "${JUDGE_LINK_DIR}/qwen3_audio_dep_base_hard_grpo_grpo100_dev_ckpt100.jsonl"
ln -sf "${ROOT}/outputs/qwen3_audio_dep_base_hard_grpo_dapo_lite100_dev_ckpt100.jsonl" "${JUDGE_LINK_DIR}/qwen3_audio_dep_base_hard_grpo_dapo_lite100_dev_ckpt100.jsonl"
...
for run in ['grpo100', 'dapo_lite100']:
    p = root / f'outputs/qwen3_audio_dep_base_hard_grpo_{run}_dev_ckpt100.jsonl'
    rows = [json.loads(x) for x in p.read_text(encoding='utf-8').splitlines() if x.strip()]
    ok = sum(bool(r.get('correct')) for r in rows)
    bad = sum(r.get('prediction_index') == -1 for r in rows)
    lines.append(f'{run}: strict={ok/len(rows):.4f} ({ok}/{len(rows)}) parse_bad={bad}')
judge = root / 'outputs/analysis/rescore_choice_matching/grpo_minimal_hard_base_qwen3_judge/qwen3_judge_parsebad_summary.json'
if judge.exists():
...
notify "DCASE base GRPO queue finished" "${summary}"
echo "${summary}"
