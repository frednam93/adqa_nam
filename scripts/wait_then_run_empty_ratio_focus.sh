...
OUT_DIR=${ROOT}/outputs/analysis/rescore_choice_matching/empty_ratio_focus_qwen3_judge
...
if [[ -n "${WAIT_PID}" ]]; then
  echo "==== $(date '+%m%d %H:%M:%S') waiting for pid=${WAIT_PID} ===="
  notify "DCASE empty-ratio focus armed" "waiting for pid=${WAIT_PID}"
...
cd "${ROOT}"
echo "==== $(date '+%m%d %H:%M:%S') prepare empty-ratio focus ===="
python3 -m dcase_adqa.prepare_empty_ratio_focus
mapfile -t runs < "${RUN_LIST}"
notify "DCASE empty-ratio focus start" "runs=${runs[*]}"
...
runs=[x.strip() for x in (root/'outputs/empty_ratio_focus_runs.txt').read_text(encoding='utf-8').splitlines() if x.strip()]
lines=[]
best=None
for run in runs:
...
judge_summary=root/'outputs/analysis/rescore_choice_matching/empty_ratio_focus_qwen3_judge/qwen3_judge_parsebad_summary.json'
...
notify "DCASE empty-ratio focus finished" "${summary}"
echo "${summary}"
