...
RUN_LIST=${ROOT}/outputs/strong_ratio_followup_runs.txt
...
echo "==== $(date '+%m%d %H:%M:%S') conditional strong-ratio follow-up armed pid=${WAIT_PID} ===="
notify "DCASE ratio follow-up armed" "waiting for strong_negative_queue pid=${WAIT_PID}"
while kill -0 "${WAIT_PID}" 2>/dev/null; do
  sleep 60
done

echo "==== $(date '+%m%d %H:%M:%S') selecting follow-up family ===="
cd "${ROOT}"
python3 -m dcase_adqa.prepare_strong_ratio_followup
mapfile -t runs < "${RUN_LIST}"
notify "DCASE ratio follow-up selected" "$(cat "${ROOT}/outputs/strong_ratio_followup_summary.json"; printf '\nruns=%s\n' "${runs[*]}")"
...
run_list=root/'outputs/strong_ratio_followup_runs.txt'
runs=[x.strip() for x in run_list.read_text(encoding='utf-8').splitlines() if x.strip()]
lines=[]
best=None
for run in runs:
...
notify "DCASE ratio follow-up finished" "${summary}"
