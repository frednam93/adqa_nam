...
run_train() {
  ...
  llamafactory-cli train "training/configs/dcase_adqa_qwen3_${run}_3k.yaml"
}

run_eval_ckpt() {
  local run=$1
  local step=$2
  local adapter="${ROOT}/outputs/qwen3_${run}_3k/checkpoint-${step}"
  local output="${ROOT}/outputs/qwen3_${run}_3k_dev_ckpt${step}.jsonl"
  if [[ ! -d "${adapter}" ]]; then
...
cd "${ROOT}"
echo "==== $(date '+%m%d %H:%M:%S') prepare/register rebalanced manifests ===="
python3 -m dcase_adqa.prepare_rebalanced_sft
python3 -m dcase_adqa.register_rebalanced_sft
notify "DCASE rebalanced SFT queue start" "v1/v2/v3 manifests prepared"
...
for run in "${runs[@]}"; do
  ...
  summary=$(
    for step in 1000 2000 3000; do
      p="${ROOT}/outputs/qwen3_${run}_3k_dev_ckpt${step}.jsonl"
...
for run in runs:
    for step in [1000, 2000, 3000]:
        p = root / f'outputs/qwen3_{run}_3k_dev_ckpt{step}.jsonl'
        if not p.exists():
            continue
        rows = [json.loads(x) for x in p.read_text(encoding='utf-8').splitlines() if x.strip()]
        ok = sum(bool(r.get('correct')) for r in rows)
...
echo "${summary}"
notify "DCASE rebalanced SFT queue finished" "${summary}"
