...
GRPO_MANIFEST=${ROOT}/outputs/ablation_manifests/train_strong_audio_dependent_grpo.jsonl
OUT_BASE=${ROOT}/outputs/grpo_minimal
JUDGE_LINK_DIR=${ROOT}/outputs/grpo_minimal_judge_inputs
JUDGE_OUT=${ROOT}/outputs/analysis/rescore_choice_matching/grpo_minimal_qwen3_judge
...
run_eval() {
  ...
  local output=${ROOT}/outputs/qwen3_audio_dep_full_strong_empty5_${run}_dev_ckpt100.jsonl
  cd "${ROOT}"
...
python3 -m dcase_adqa.prepare_grpo_manifest --output "${GRPO_MANIFEST}"
...
run_grpo grpo100
run_eval grpo100
notify "DCASE GRPO100 done" "$(summarize_eval_file "${ROOT}/outputs/qwen3_audio_dep_full_strong_empty5_grpo100_dev_ckpt100.jsonl")"
sleep 3

run_grpo dapo_lite100 --dapo-lite
run_eval dapo_lite100
notify "DCASE DAPO-lite100 done" "$(summarize_eval_file "${ROOT}/outputs/qwen3_audio_dep_full_strong_empty5_dapo_lite100_dev_ckpt100.jsonl")"
sleep 3

rm -rf "${JUDGE_LINK_DIR}"
mkdir -p "${JUDGE_LINK_DIR}" "${JUDGE_OUT}"
ln -sf "${ROOT}/outputs/qwen3_audio_dep_full_strong_empty5_grpo100_dev_ckpt100.jsonl" "${JUDGE_LINK_DIR}/qwen3_audio_dep_full_strong_empty5_grpo100_dev_ckpt100.jsonl"
ln -sf "${ROOT}/outputs/qwen3_audio_dep_full_strong_empty5_dapo_lite100_dev_ckpt100.jsonl" "${JUDGE_LINK_DIR}/qwen3_audio_dep_full_strong_empty5_dapo_lite100_dev_ckpt100.jsonl"
...
judge = root / "outputs/analysis/rescore_choice_matching/grpo_minimal_qwen3_judge/qwen3_judge_parsebad_summary.json"
if judge.exists():
